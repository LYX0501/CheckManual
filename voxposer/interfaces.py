from LMP import LMP
from vutils import get_clock_time, normalize_vector, pointat2quat, bcolors, Observation, VoxelIndexingWrapper
import numpy as np
...
class LMP_interface():
  ...
  def __init__(self, env, lmp_config, controller_config, planner_config, env_name='rlbench'):
    ...
    self._map_size = int(self._cfg['map_size'])
  ...
  def detect(self, obj_name):
    ...
    # if obj_name in self.first_obs.keys(): # 只返回第一帧的观测
    #   print('use first obs',obj_name)
    #   return self.first_obs[obj_name]
    # else:
    #   print('use current obs:', obj_name)
    
    """return an observation dict containing useful information about the object"""
    if obj_name.lower() in EE_ALIAS:
      print("detect() | obj_name.lower() in EE_ALIAS")
      obs_dict = dict()
      obs_dict['name'] = obj_name
      obs_dict['position'] = self.get_ee_pos()  # voxel下坐标
      ...
      print("detect() | obj_name.lower() in TABLE_ALIAS")
    ...
    print("position center:", obs_dict['position'])
    object_obs = Observation(obs_dict)
    # if obj_name not in self.first_obs.keys():
    #   self.first_obs[obj_name] = object_obs
    return object_obs
  ...
  def execute(self, movable_obs_func, affordance_map=None, avoidance_map=None, rotation_map=None,
    ...
    if affordance_map is not None:
      ...
      for plan_iter in range(self._cfg['max_plan_iter']):
        ...
        timetmp = time.time()
        save_dir = self._env.real_env.task.sapien_env.save_dir
        if not os.path.exists(f'{save_dir}/{timetmp}_{obj_name}'):
          os.mkdir(f'{save_dir}/{timetmp}_{obj_name}')

        # # Set initial gripper pose
        # ee_pos_world_start = traj_world[0][0]
        # ee_rot_world_start = traj_world[0][1]
        # ee_pose_world_start = np.concatenate([ee_pos_world_start, ee_rot_world_start])
        # gripper_state_start = traj_world[0][3]
        # print("ee_pose_world_start", ee_pose_world_start)
        # self._env.apply_action(np.concatenate([ee_pose_world_start, [gripper_state_start]]), 'Set')


        # Save observation before executing action
        obs_tmp = self._env.real_env.task.sapien_env.get_observation()
        cv2.imwrite(f'{save_dir}/{timetmp}_{obj_name}/before.png',cv2.cvtColor( obs_tmp['image'],cv2.COLOR_BGR2RGB))
        
        # Draw waypoints
        waypoint_img = cv2.imread(f'{save_dir}/{timetmp}_{obj_name}/before.png')
        for waypoint in traj_world:
          self.draw_traj(waypoint_img,waypoint)
        cv2.imwrite(f'{save_dir}/{timetmp}_{obj_name}/waypoint.png',waypoint_img)
        
        # Draw target point
        target_img = cv2.imread(f'{save_dir}/{timetmp}_{obj_name}/before.png')
        self.draw_traj(target_img,traj_world[-1])
        cv2.imwrite(f'{save_dir}/{timetmp}_{obj_name}/target.png',target_img)
        
        
        # for i, waypoint in enumerate(traj_world[-1:]):
        ...
        #   obs_tmp = self._env.real_env.task.sapien_env.get_observation()
        #   cv2.imwrite(f'{save_dir}/{timetmp}_{obj_name}/after_{i}.png',cv2.cvtColor( obs_tmp['image'],cv2.COLOR_BGR2RGB))
    ...
    if not object_centric:
      ...
      print("ee_pose_world", ee_pose_world)
      print("gripper_state", gripper_state)
      self._env.apply_action(np.concatenate([ee_pose_world, [gripper_state]]), 'Simple')
  ...
  def cm2index(self, cm, direction):
    if isinstance(direction, str) and direction == 'x':
      ...
      print("Raw cm2index direction: ", direction)
      direction = normalize_vector(direction)
      print("Processed cm2index direction: ", direction)
      x_cm = cm * direction[0]
      y_cm = cm * direction[1]
      z_cm = cm * direction[2]
      ...
      print("xyz_index", np.array([x_index, y_index, z_index]))
  ...
  def vec2quat(self, vector):
    ...
    # 如果轴向量非常小，意味着 z 轴和 v 方向相同或反向
    if axis_norm < 1e-6:
        return np.array([1, 0, 0, 0])  # 没有旋转
    ...
    # 构造四元数
    w = np.cos(np.arccos(cos_theta) / 2)
    x, y, z = axis * np.sin(np.arccos(cos_theta) / 2)
    
    # 返回四元数
    return np.array([w, x, y, z])
    return transforms3d.quaternions.mat2quat(transforms3d.euler.euler2mat(vector[0], vector[1], vector[2]))
  ...
  def _get_default_voxel_map(self, type='target'):
    ...
    def fn_wrapper():
      if type == 'target':
        ...
        print("self._env.get_ee_quat()", self._env.get_ee_quat())
      else:
  ...
  def draw_traj(self,result,waypoint):
    
    world_pose = np.eye(4)
    world_pose[:3,3] = waypoint[0]
    world_pose[:3,:3] = self.quaternion_to_rotation_matrix(waypoint[1])  # world space
    
    w2c = np.linalg.inv(self._env.latest_obs['c2w'])
    pose = w2c@world_pose
    
    rotation_vector,_ = cv2.Rodrigues(pose[:3,:3])
    # cv2.drawFrameAxes(result, np.array([[camera_intrinsic['fx'], 0, camera_intrinsic['cx']], [0,camera_intrinsic['fy'], camera_intrinsic['cy']], [0, 0, 1]]), np.array([0.,0.,0.,0.,0.]), rotation_vector, target_pose[:3,3], 0.05)
    axis_length = 0.05
    # 定义3D坐标轴的方向向量 (世界坐标系下的X、Y、Z方向)
    axis_points_3d = np.float32([[axis_length, 0, 0], 
                        [0, axis_length, 0],   # Y 轴
                        [0, 0, axis_length],   # Z 轴
                        [0, 0, 0]])  # 原点
    # 将3D点投影到2D图像坐标上
    axis_points_2d, _ = cv2.projectPoints(axis_points_3d, rotation_vector, pose[:3,3], self._env.latest_obs['cam_info']['K'], np.array([0.,0.,0.,0.,0.]))
    # 提取2D坐标点
    axis_points_2d = np.int32(axis_points_2d).reshape(-1, 2)
    origin = tuple(axis_points_2d[3])  # 原点
    x_end = tuple(axis_points_2d[0])   # X轴终点
    y_end = tuple(axis_points_2d[1])   # Y轴终点
    z_end = tuple(axis_points_2d[2])   # Z轴终点
    # 使用cv2.line手动绘制坐标轴
    cv2.line(result, origin, x_end, (0, 0, 255), 2)  # X轴 (红色), 线条宽度为1
    cv2.line(result, origin, y_end, (0, 255, 0), 2)  # Y轴 (绿色), 线条宽度为1
    cv2.line(result, origin, z_end, (255, 0, 0), 2)  # Z轴 (蓝色), 线条宽度为1
