...
def save_pil_image(args, path, array):
    if args.save_vis:
        Image.fromarray(array).save(path)


def save_cv_image(args, path, array):
    if args.save_vis:
        cv2.imwrite(path, array)


def save_gif(args, path, frames):
    if args.save_vis and frames:
        imageio.mimsave(path, frames)


def action_timed_out(start_time, max_seconds):
    return max_seconds is not None and max_seconds > 0 and (time.time() - start_time) > max_seconds


def top_grasps(grasp_data, max_trials):
    zipped = list(zip(grasp_data["gg_score"], grasp_data["gg_pos"], grasp_data["gg_rot"]))
    zipped.sort(key=lambda item: item[0], reverse=True)
    if max_trials is not None and max_trials > 0:
        zipped = zipped[:max_trials]
    return zipped
...
def estimate_pose(args, shape_id, manual_subdir, view="front", img_size=1024):
    ...
    sample_metadata = infer_sample_metadata(source_sample_dir)
...
def push_button(flog, env, args, save_dir, shape_id, link_name, part_name, prev_joint_angles, view, img_size):
    ...
    phi, theta = get_camera_angles(view)
    camera_size = args.button_camera_size
    ...
    save_cv_image(args, os.path.join(save_dir, "start_pose.png"), marked_rgb)
    ...
    try:
        ...
        for attempt_idx, (final_dist, move_steps, settle_steps) in enumerate(button_attempts, start=1):
            if action_timed_out(action_start, args.max_action_seconds):
                flog.write(f"{shape_id}_{link_name}_{part_name} timed out before attempt_{attempt_idx}\n")
                break
            ...
            if action_timed_out(action_start, args.max_action_seconds):
                flog.write(f"{shape_id}_{link_name}_{part_name} timed out after attempt_{attempt_idx}\n")
                break
        ...
        save_pil_image(args, os.path.join(save_dir, "target_pose.png"), rgb_final_pose)
        save_gif(args, os.path.join(save_dir, "apprchimgs.gif"), apprchimgs)
...
def rotate_knob(flog, env, args, save_dir, shape_id, link_name, part_name, rotation_degrees, prev_joint_angles, view, img_size):
    ...
    phi, theta = get_camera_angles(view)
    cam = Camera(env, dist=5, image_size=args.exec_camera_size, phi=phi, theta=theta)
    ...
    if args.save_vis:
        ...
        o3d.io.write_point_cloud(os.path.join(save_dir, "point_cloud.ply"), pcd)
    ...
    try:
        ...
        return prev_joint_angles, success_flag
    if action_timed_out(action_start, args.max_action_seconds):
        flog.write(f"{shape_id}_{link_name}_{part_name} timed out before grasp evaluation\n")
        env.scene.remove_articulation(robot.robot)
        return prev_joint_angles, success_flag
    ...
    if action_timed_out(action_start, args.max_action_seconds):
        flog.write(f"{shape_id}_{link_name}_{part_name} timed out before rotation\n")
        updated_joint_angles = env.get_object_qpos()
        env.scene.remove_articulation(robot.robot)
        return updated_joint_angles, success_flag
    ...
    save_pil_image(args, os.path.join(save_dir, "target_pose_rotated.png"), rgb_final_pose_rotated)

    # # Save the approach images as a GIF
    save_gif(args, os.path.join(save_dir, "apprchimgs_rotated.gif"), apprchimgs_rotated)
...
def manip_slider_or_door(flog, env, args, save_dir, manual_subdir, shape_id, link_name, part_name, prev_joint_angles, joint2world_dict, view, img_size, use_cached_grasp=False):
    ...
    if use_cached_grasp and os.path.exists(cache_grasp_path):
        ...
        if not args.grasp_server_url:
            flog.write(
                f"{shape_id}_{link_name}_{part_name} missing grasp cache and no --grasp_server_url; skip action\n"
            )
            p.disconnect()
            return prev_joint_angles, success_flag
    ...
    selected_grasps = top_grasps(grasp_data, args.max_grasp_trials)
    gg_score = [item[0] for item in selected_grasps]
    gg_pos = [np.array(item[1]) for item in selected_grasps]
    gg_rot = [np.array(item[2]) for item in selected_grasps]
    flog.write(
        f"{shape_id}_{link_name}_{part_name} evaluating {len(gg_score)}/{len(grasp_data.get('gg_score', []))} grasp candidates\n"
    )
    ...
    for gg_idx in range(gg_len):
        if action_timed_out(action_start, args.max_action_seconds):
            flog.write(f"{shape_id}_{link_name}_{part_name} timed out before grasp trial {gg_idx + 1}\n")
            break
        gg_idx_ = gg_idx + 1
        ...
        if args.save_vis:
            ...
            apprchimgs = []
            robot.move_to_target_pose(target_gg_rotmat, 1000, cam=cam, vis_gif=False, visu=False)
        robot.wait_n_steps(500)
        ...
        robot.wait_n_steps(500)
        if action_timed_out(action_start, args.max_action_seconds):
            flog.write(f"{shape_id}_{link_name}_{part_name} timed out after grasp trial {gg_idx + 1}\n")
            if robot_existence:
                env.scene.remove_articulation(robot.robot)
                robot_existence = False
            break
        ...
        save_pil_image(args, os.path.join(save_dir, f"gg_pose_{gg_idx}.png"), rgb_gg_pose)
        save_gif(args, os.path.join(save_dir, f"apprchimgs_{gg_idx}.gif"), apprchimgs)
    ...
    if success_grasp:
        if "door" in part_name:
            if action_timed_out(action_start, args.max_action_seconds):
                flog.write(f"{shape_id}_{link_name}_{part_name} timed out before door opening motion\n")
                updated_joint_angles = env.get_object_qpos()
            ...
            if args.save_vis:
                ...
                pullupimgs = []
                robot.move_to_target_pose(opendoor_rotmat, 4000, cam=cam, vis_gif=False, visu=False)
            ...
            save_pil_image(args, os.path.join(save_dir, f"pullup_{gg_idx}.png"), rgb_pullup)
            save_gif(args, os.path.join(save_dir, f"pullupimgs_{gg_idx}.gif"), pullupimgs)
            ...
            if action_timed_out(action_start, args.max_action_seconds):
                flog.write(f"{shape_id}_{link_name}_{part_name} timed out before pull motion\n")
                updated_joint_angles = env.get_object_qpos()
                if robot_existence == True:
                    env.scene.remove_articulation(robot.robot)
                p.disconnect()
            ...
            if args.save_vis:
                ...
                pullupimgs = []
                robot.move_to_target_pose(pullup_rotmat, 1000, cam=cam, vis_gif=False, visu=False)
            robot.wait_n_steps(500)
...
def parse_args():
    ...
    parser.add_argument("--max_grasp_trials", default=5, type=int)
