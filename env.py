...
class Env(object):
    ...
    def load_object(self, urdf, material, rotation="front", state='closed', init_scale=1, init_pos=None, init_rot=None):
        ...
        if type(init_pos) != list:
            init_pos = [self.object_position_offset, 0, 0]
            
        if type(init_rot) != list:
            theta_radians = 0 * math.pi / 180
            w1 = math.cos(theta_radians / 2)
            x1 = 0 
            y1 = math.sin(theta_radians / 2)
            z1 = 0
            if rotation == "front":
                w, x, y, z = 1, 0, 0, 0            
            elif rotation == "left-front":
                theta_radians = 20 * math.pi / 180
                w = math.cos(theta_radians / 2)
                x = 0
                y = 0
                z = math.sin(theta_radians / 2)
            elif rotation == "right-front":
                theta_radians = -20 * math.pi / 180
                w = math.cos(theta_radians / 2)
                x = 0
                y = 0
                ...
                theta_radians = - 85 * math.pi / 180  # 90度转换为弧度
                w = math.cos(theta_radians / 2)
                x = 0  # 绕y轴旋转时，x分量为0
                y = math.sin(theta_radians / 2)
                z = 0  # 绕y轴旋转时，z分量为0
        
            w = w1 * w - x1 * x - y1 * y - z1 * z
            x = w1 * x + x1 * w + y1 * z - z1 * y
            y = w1 * y - x1 * z + y1 * w + z1 * x
            z = w1 * z + x1 * y - y1 * x + z1 * w
            init_rot = [w, x, y, z]
            print("init_rot: "+str(init_rot)+"\n")
            # self.flog.write("init_rot: "+str(init_rot)+"\n")
            
        pose = Pose(init_pos, init_rot)
    ...
    def get_link_bounding_box(self, urdf_file, target_link_name):
        # create a Sapien scene and pose loader
        scene = self.engine.create_scene()
        pose_loader = scene.create_urdf_loader()
